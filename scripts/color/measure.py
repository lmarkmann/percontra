...
CALC = str(pathlib.Path.home() / "Documents/.parked/Skills/frontend-skills/tools/color-themes/calc.py")

def run(args):
    out = subprocess.run(["uv", "run", "--script", CALC, *args],
...
# --- neutral ramp ---
all_colors = []
for step in P.NEUTRAL:
    c = P.neutral(step); result["neutral"][step] = {"oklch": c}; all_colors.append(c)

# --- accents ---
for d in ("verdigris", "ink"):
    result["accents"][d] = {}
    for step in ("400","500","600","700","800","900"):
        c = P.accent(d, step); result["accents"][d][step] = {"oklch": c}; all_colors.append(c)

# --- statuses ---
for theme in ("light", "dark"):
    result["status"][theme] = {}
    for name in P.STATUS_HUE:
        entry = {r: P.status(theme, name, r) for r in ("mark", "tint", "text")}
...
def put_hex(d):
    for v in d.values():
        if isinstance(v, dict):
            if "oklch" in v: v["hex"] = hx.get(v["oklch"])
            else: put_hex(v)
for group in ("neutral", "accents"): put_hex(result[group])
for theme in result["status"]:
    for name, roles in result["status"][theme].items():
        result["status"][theme][name] = {r: {"oklch": c, "hex": hx.get(c)} for r, c in roles.items()}

for c in sorted(set(all_colors)):
    if not gamut_ok(c): result["gamut_failures"].append(c)

# --- pairs that must pass ---
n = P.neutral
LIGHT_BG, LIGHT_CARD, LIGHT_FG, LIGHT_MUTED, LIGHT_BORDER = n("50"), n("100"), n("800"), n("700"), n("200")
DARK_BG,  DARK_CARD,  DARK_FG,  DARK_MUTED,  DARK_BORDER  = n("950"), n("900"), n("100"), n("500"), n("700")

def add(label, fg, bg, need, kind):
    r = contrast(fg, bg); r.update(label=label, need=need, kind=kind)
...
for d in ("verdigris", "ink"):
    add(f"light/{d}: primary-foreground on primary", n("50"), P.accent(d, "800"), 4.5, "text")
    add(f"light/{d}: primary as text on background", P.accent(d, "800"), LIGHT_BG, 4.5, "text")
    add(f"dark/{d}: primary-foreground on primary", n("950"), P.accent(d, "500"), 4.5, "text")
    add(f"dark/{d}: primary as text on card", P.accent(d, "500"), DARK_CARD, 4.5, "text")

for theme, card in (("light", LIGHT_CARD), ("dark", DARK_CARD)):
    for name in P.STATUS_HUE:
        mark, tint, text = (P.status(theme, name, r) for r in ("mark", "tint", "text"))
...
print(f"pairs measured: {len(result['pairs'])}   failing: {len(fails)}   out-of-gamut: {len(result['gamut_failures'])}")
for p in fails:
