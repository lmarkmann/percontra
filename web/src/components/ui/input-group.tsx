...
const inputGroupAddonVariants = cva(
	"flex icon-default h-auto cursor-text items-center justify-center gap-2 py-1.5 text-caption font-medium text-muted-foreground select-none group-data-[disabled=true]/input-group:opacity-50 [&>kbd]:rounded-[calc(var(--radius)-5px)]",
	{
		variants: {
			align: {
				"inline-start":
...
const inputGroupButtonVariants = cva(
	"flex items-center gap-2 text-caption shadow-none",
	{
		variants: {
			size: {
				xs: "h-6 gap-1 rounded-inner-sm px-1.5 [&>svg:not([class*='size-'])]:size-3.5",
				sm: "h-7 gap-1 rounded-inner-md px-2 text-caption [&>svg:not([class*='size-'])]:size-3.5",
				"icon-xs": "size-6 rounded-inner-sm p-0 has-[>svg]:p-0",
				"icon-sm": "size-7 rounded-inner-md p-0 has-[>svg]:p-0",
			},
		},
		defaultVariants: {
			size: "xs",
...
function InputGroupText({ className, ...props }: React.ComponentProps<"span">) {
	return (
		<span
			className={cn(
				"flex icon-default items-center gap-2 text-caption text-muted-foreground [&_svg]:pointer-events-none",
				className,
			)}
			{...props}
		/>
